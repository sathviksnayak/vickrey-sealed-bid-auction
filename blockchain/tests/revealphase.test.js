...
describe("Reveal Phase", function () {
  it("should allow a valid bidder to reveal", async function () {
    ...
    await auction.connect(alice).commitBid(bidHash, { value: amount });

    await auction.connect(bob).commitBid(bidHash, { value: amount });

    await time.increase(3601);

    await expect(auction.connect(alice).revealBid(amount, salt)).to.emit(
      auction,
      "BidRevealed"
    );
